...
Produce a JSON response with these exact fields:
{
  ...
  // IMPORTANT: always include EXACTLY 3 alternative_actions — never fewer
...
def run_nba_agent(state: CollectionWorkflowState) -> CollectionWorkflowState:
    ...
    event_bus.emit(state["workflow_id"], "agent_update", {"agent": "nba", "stage": 3, "status": "running", "elapsed_ms": None, "error": None})
...
TRIGGER CONTEXT: {state.get('trigger_context', 'routine_review')}"""
        ...
        event_bus.emit(state["workflow_id"], "agent_update", {"agent": "nba", "stage": 3, "status": "completed", "elapsed_ms": elapsed_ms, "error": None})
    except Exception as e:
        elapsed_ms = int((datetime.now(timezone.utc) - started_at).total_seconds() * 1000)
        state["agent_statuses"]["nba"].update({
            "status": "error", "error": str(e), "elapsed_ms": elapsed_ms,
        })
        event_bus.emit(state["workflow_id"], "agent_update", {"agent": "nba", "stage": 3, "status": "error", "elapsed_ms": elapsed_ms, "error": str(e)})
